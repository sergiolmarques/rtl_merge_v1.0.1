...
VPP := v++
VPP_PFLAGS := 
CMD_ARGS = -x $(BUILD_DIR)/merge.xclbin
SDCARD := sd_card
...
BINARY_CONTAINERS += $(BUILD_DIR)/merge.xclbin
BINARY_CONTAINER_merge_OBJS += $(TEMP_DIR)/merge.xo
...
# Building kernel
$(BUILD_DIR)/merge.xclbin: $(BINARY_CONTAINER_merge_OBJS)
	mkdir -p $(BUILD_DIR)
ifeq ($(HOST_ARCH), x86)
	$(VPP) $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(BUILD_DIR)/merge.link.xclbin' $(+)
	$(VPP) -p $(BUILD_DIR)/merge.link.xclbin -t $(TARGET) --platform $(PLATFORM) --package.out_dir $(PACKAGE_OUT) -o $(BUILD_DIR)/merge.xclbin
else
	$(VPP) $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(BUILD_DIR)/merge.xclbin' $(+)
endif
...
ifneq ($(HOST_ARCH), x86)
	$(VPP) $(VPP_PFLAGS) -p $(BUILD_DIR)/merge.xclbin -t $(TARGET) --platform $(PLATFORM) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) -o merge.xclbin
endif
